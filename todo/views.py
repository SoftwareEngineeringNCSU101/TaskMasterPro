...
"""
Views module for Todo application handling all request-response logic.
Contains functions for managing todo lists, items, templates, and user authentication.
"""
...
from todo.models import (
    List, ListItem, Template, 
    ListTags, SharedUsers, SharedList
)
from todo.forms import NewUserForm
...
def index(request, list_id=0):
    ...
    Args:
        ...
        list_id: Optional list ID to filter specific list
    
    Returns:
        Rendered template with todo lists context
    """
    if not request.user.is_authenticated:
        return redirect("/login")
...
def todo_from_template(request):
    """
    Create a new todo list from template and redirect to homepage.
    
    Args:
        request: HTTP request object
    
    Returns:
        Redirect to todo homepage
    """
    if not request.user.is_authenticated:
        return redirect("/login")
...
def remove_list_item(request):
    ...
    Args:
        request: HTTP request object with list item ID
    ...
    try:
        ...
        print(f"Error removing list item: {str(err)}")
...
def update_list_item(request, item_id):
    ...
    Args:
        request: HTTP request object with updated text
    ...
    try:
        ...
        print(f"Error updating list item: {str(err)}")
        
    return redirect("/")

@csrf_exempt
def add_new_list_item(request):
    """
    Add a new todo list item.
    
    Args:
        request: HTTP request object with item details
    
    Returns:
        JsonResponse with new item ID
    """
    if not request.user.is_authenticated:
        return redirect("/login")
    ...
    create_on_time = datetime.datetime.fromtimestamp(body['create_on'])
    finished_on_time = datetime.datetime.fromtimestamp(body['create_on'])
    ...
    try:
        with transaction.atomic():
            ...
            todo_list_item.save()
            return JsonResponse({'item_id': todo_list_item.id})
    except IntegrityError:
        return JsonResponse({'item_id': -1})

@csrf_exempt
def mark_list_item(request):
    """
    ...
    Returns:
        JsonResponse with updated item details
    """
    if not request.user.is_authenticated:
        return redirect("/login")
    ...
    is_done_str = str(body['is_done'])
    list_item_is_done = not (
        is_done_str == "0" or 
        is_done_str.lower() == "false"
    )
    
    finished_on_time = datetime.datetime.fromtimestamp(body['finish_on'])
    
    try:
        with transaction.atomic():
            query_list = List.objects.get(id=body['list_id'])
            query_item = ListItem.objects.get(id=body['list_item_id'])
            query_item.is_done = list_item_is_done
            query_item.finished_on = finished_on_time
            query_item.save()
...
@csrf_exempt
def get_list_tags_by_userid(request):
    """
    Get all list tags for a user.
    
    Args:
        request: HTTP request object
    
    Returns:
        JsonResponse with list tags
    """
    if not request.user.is_authenticated:
        return redirect("/login")
...
@csrf_exempt
def get_list_item_by_name(request):
    """
    Get a todo list item by name.
    
    Args:
        request: HTTP request object with list and item details
    
    Returns:
        JsonResponse with item details
    """
    if not request.user.is_authenticated:
        return redirect("/login")
...
@csrf_exempt
def get_list_item_by_id(request):
    """
    Get a todo list item by ID.
    
    Args:
        request: HTTP request object with item ID
...
@csrf_exempt
def create_new_todo_list(request):
    """
    Create a new todo list.
    
    Args:
        request: HTTP request object with list details
    ...
    try:
        with transaction.atomic():
            todo_list = List.objects.create(
                user_id_id=request.user.id,
                title_text=body['list_name'],
                created_on=create_on_time,
                updated_on=create_on_time,
                list_tag=body['list_tag']
            )
            ...
            if body['shared_user']:
                user_list = body['shared_user'].split()
                
                for username in user_list[:]:
                    try:
                        query_user = User.objects.get(username=username)
                        shared_list = SharedList.objects.get(user=query_user)
                        shared_list.shared_list_id += f"{todo_list.id} "
                        shared_list.save()
                    except User.DoesNotExist:
        ...
        print(f"Error creating todo list: {str(err)}")
        return HttpResponse("Request failed when operating on database")
